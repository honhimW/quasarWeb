      ...
      post(replaceUrl, this.sendMethod, header, JSON.stringify(JSON.parse(this.reqBody.code))).then(resp => {
        if (resp.status === 200 && resp.data.httpStateCode === 200) {
          if (resp.data.body !== null && resp.data.body !== undefined) {
            this.respBody = {
              code: JSON.stringify(resp.data.body, null, 2)
            }
          } else {
            this.respBody = {
        ...
        this.$q.notify({
          type: resp.data.httpStateCode.toString().startsWith('2') ? 'positive' : resp.data.httpStateCode.toString().startsWith('4') ? 'warning' : 'negative',
          message: resp.data.httpStateCode + ', ' + resp.data.cost + 'ms',
