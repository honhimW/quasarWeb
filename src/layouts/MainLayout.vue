...
<script>
import { getAllInfo, getRequest } from 'src/utils/getRequest'
  ...
  methods: {
    ...
    confirmCP () {
      ...
      switch (this.createType) {
        case 'New Project':
          promis = Opt.createProject(this.projectName, this.projectDesc).then(resp => {
            if (resp.status === 200) {
              this.refreshTree()
            } else {
              this.$q.notify({
    ...
    init () {
      getAllInfo().then(resp => {
        ...
        this.customize = this.toCus(resp.data.projectList)
      })
    },
    getOpt (type) {
      switch (type) {
    ...
    selectedChange (val) {
      ...
      if (reqNode === undefined || reqNode === null) {
        ...
        getRequest(reqNode.project, reqNode.group, reqNode.label, reqNode.uuid).then(resp => {
          if (resp.status === 200) {
            if (resp.data.retn === '0000') {
              this.$root.$emit('openRequest', resp.data.body, reqNode.project, reqNode.group, reqNode.label, reqNode.uuid)
            } else {
              this.$q.notify({
                type: 'negative',
